...
int getBackgroundThreshold(VideoCapture cap, int limit){
	...
	while (i < limit) {		
		...
		//cout << "Change in Green Chi is " << g_changeCHI << endl;
		//cout << "Change in Blue Chi is " << b_changeCHI << endl;
		//cout << "Change in  Chi is " << changeCHI << endl;
		if (changeCHI < 1){
		...
		Mat b_hist_back, g_hist_back, r_hist_back;/*This statement here saved me*/
		if (changeCHI >0 && (changeCHI) < 4000 ){
			...
			//Mat r = r_hist_curr.clone();
			//Mat g = g_hist_curr.clone();
			//Mat b = b_hist_curr.clone();
			
			//r_hist_curr.copyTo(r_hist_back);
			//g_hist_curr.copyTo(g_hist_back);
			//b_hist_curr.copyTo(b_hist_back);
			...
			double changeCHI = B_WEIGHT*b_changeCHI + R_WEIGHT*r_changeCHI + G_WEIGHT*g_changeCHI;
			cout << "Change in  Chi after updation is " << changeCHI << endl;
			int xx;
			cin >> xx;
			cin.clear();
			cin.ignore(INT_MAX, '\n');*/
	...
	thr = mean + (int)stdev;
	//cout << "Square sum is " << sq_sum << endl;
	//cout << "Mean is " << mean << endl;
	//cout << "Std is " << stdev << endl;

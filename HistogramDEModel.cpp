...
void ShowResults(vector<Vec3f> circles,Mat src_display){
	...
	// shows the results
	imshow("Original Video", src_display);
...
void ShowCircles(char str[],vector<Vec3f> circles, Mat src_display){
	Mat vehicle = Mat::zeros(200, 1260, CV_8UC3);
	Mat clearedVehicle = Mat::zeros(200, 1260, CV_8UC3);
	Mat display = src_display.clone();
	for (size_t i = 0; i < circles.size(); i++)
...
void OptimizeDE(Mat src, Mat src_gray,vector<Vec3f> circles,char infile[],char outfile[]){
	vector<Vec3f> deCircles = DE(src, src_gray, circles, infile, outfile);
	ShowCircles("Detected by DE", deCircles, src);
	cvWaitKey(0);
}
void DetectCircles(Mat src){
	...
	ShowResults(alignedCircles, src);
	//Call DE module
	OptimizeDE(src, src_gray, alignedCircles,"in.txt", "out.txt");
	//cvWaitKey(0);
}

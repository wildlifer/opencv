...
{
	VideoCapture cap("videos/1.6.mp4");
	...
	while (1){
		...
		imshow("Original Video", hsv_nextframe);
		cvMoveWindow("Original Video", 0, 0);
		for (int i = 0; i < 4; i++)
		{
			int compare_method = i;
			double change = compareHist(hist_background, hist_nextframe, compare_method);
			//printf(" Method [%d]  Change for method ,  %f \n", i, change);
			switch (i){
			case 0:outFile << change; break;
			case 1:outFile2 << change; break;
			case 2:outFile3 << change; break;
			case 3:outFile4 << change; break;
			}
			printf("--------------------------------------------\n");

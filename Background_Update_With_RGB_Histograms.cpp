...
double MIN_DETECTABLE_AREA = 100;


long whites = 0;
const int INITIAL_LEARN_FRAME_LIMIT = 15;
...
int main(int argc, char** argv)
{
	...
	vector<int> hist_diff_store(LEARN_FRAME_LIMIT);
	while (1) {
		//clear planes
		bgr_planes_curr_frame.clear();
		//new frame
		...
		//Store the difference between histograms
		...
		if (changeCHI < CHISQUARE_THRES){
			...
			if (frameLearnCounter < LEARN_FRAME_LIMIT){
				hist_diff_store[frameLearnCounter] = changeCHI;
			...
			if (frameLearnCounter == LEARN_FRAME_LIMIT && max_area<MIN_DETECTABLE_AREA){
				frameLearnCounter = 0;
				//matStore.clear();
			...
			//Draw only when contours found
			if (contours.size() > 0 && max_area > MIN_VEHICLE_AREA){			
				for (int i = 0; i < contours.size(); i++)
				{
			...
			}//end of if (contours.size() > 0 && max_area > MIN_VEHICLE_AREA)
			else if (contours.size() > 0 && max_area < MIN_DETECTABLE_AREA){//if counters found and threshold is crossed with min area, update the background			
				//Re-initialize the matrix. Very important
				Mat b_hist_back, g_hist_back, r_hist_back;
				frameLearnCounter = 0;
				//copy current frame to the background
				...
				g_hist_back = g_hist_curr.clone();
				b_hist_back = b_hist_curr.clone();
				cout << "Updating Background as minimum area condition not satisfied" << endl;
				//pause();
			}//end of if (contours.size() > 0 && max_area < MIN_DETECTABLE_AREA)
			else if (contours.size() == 0){//if no counters and threshold is crossed, update the background			
				//Re-initialize the matrix. Very important
				Mat b_hist_back, g_hist_back, r_hist_back;
				frameLearnCounter = 0;
				//copy current frame to the background

...
{
	CvCapture* capture = cvCreateFileCapture("videos/new.mp4");
	...
	long whites = 0;
	long thresholdWhitecount = 3;
	int counter = 0;
	long framecounter = 0;
	std::map<long, IplImage*> m;
	//IplImage* tempFrame = cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 3);
	//IplImage* smallFrame = cvCreateImage(cvSize(firstFrame->width / 2, firstFrame->depth / 2), IPL_DEPTH_8U, 1);
	...
	while (1) {
		...
		//resizeWindow("Original Video", 400, 600);
		
		cvCvtColor(frame, frameGray, CV_BGR2GRAY);
		
		whites = cvCountNonZero(frameGray);
		cvShowImage("Gray Video", frameGray);
		cvMoveWindow("Gray Video", 600, 0);
		framecounter++;
		//cout << "Before continue" << endl;
		cout << "Initial Whites are " << initialWhites << endl;
		cout << "Current Whites are " << whites << endl;
		cout << "Difference Whites are " << initialWhites - whites << endl;

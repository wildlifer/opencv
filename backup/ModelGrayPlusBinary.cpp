...
{
	CvCapture* capture = cvCreateFileCapture("videos/2.2.mp4");
	long initWhites = 0;
	long whites = 0;
	...
	while (1) {
		...
		if (abs(initWhites - whites) < diffWhitesThreshould){
			oldBackWhites = 0;
		}
		if (abs(initWhites - whites) < noisyWhitesThreshould || abs(initWhites - whites)>upperThreshouldWhiteDifference){// || (initWhitesGray - whitesGray) < grayNoisyWhitesThreshould){
			buffer = false;
			//oldBackWhites = 0;
			cout << "********Toggling buffer to false at lower threshold with abs(initWhites - whites)->> " << abs(initWhites - whites) << endl;
		//	cout << "******* and (initWhitesGray - whitesGray)->> " << (initWhitesGray - whitesGray) << endl;
			//upperThreshouldWhiteDifference += upperThreshouldWhiteDifferenceIncrement; //decrement it at an appropriate place too.
			//continue;
		}
		//else if (abs(initWhites - whites)>upperThreshouldWhiteDifference){
			//buffer = false;
			//cout << "********Toggling buffer to false at upper threshold with abs(initWhites - whites)->> " << abs(initWhites - whites) << endl;
		...
		if ( ((initWhitesGray - whitesGray)> threshouldGrayWhiteDifference && buffer==false) || 
			(abs(initWhites - whites) > threshouldWhiteDifference  && 
			abs(whitesPrevFrame - whites)>400 && buffer == false)){//Check 1
			cout << "+++++++++++++++++++++" << endl;
			cout << "change detected" << endl;
			cout << "+++++++++++++++++++++" << endl;
			cout << "abs(whitesPrevFrame - whites) are  " << abs(whitesPrevFrame - whites) << endl;
			cout << "oldBackWhites are " << oldBackWhites << endl;
			...
			if (abs(whitesPrevFrame - initWhites) > abs(whites - initWhites)){ //&& //Check if the whites have started reducing
				cout << "FIRST "<<endl;
				if (abs(whitesPrevFrame - whites)>600 && abs(whitesPrevFrame - whites) < 1000){// && //Check if previous previous frame and current are appreciably different but within bounds
				cout << "SECOND " << endl;
				//cvWaitKey(0);
				if (abs(oldBackWhites - abs(whitesPrevFrame - whites))>200){//Check if the current frame if different from last Best frame
					cout << "***********************" << endl;

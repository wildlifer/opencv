...
long noisyWhitesThreshould = 1000;
long grayNoisyWhitesThreshould = 2;
...
{
	CvCapture* capture = cvCreateFileCapture("videos/1.3.mp4");
	long initWhites = 0;
	long whites = 0;
	...
	while (1) {
		...
		//cvWaitKey(0);
		if ((initWhites - whites) < noisyWhitesThreshould || (initWhitesGray - whitesGray) < grayNoisyWhitesThreshould){
			buffer = false;
		...
		if ((initWhites - whites) > threshouldWhiteDifference  && abs(whitesPrevFrame - whites)>300 && buffer == false){//Check 1
			cout << "+++++++++++++++++++++" << endl;
			...
			if ((whitesPrevFrame - whites) < 600 && abs(oldBackWhites - abs(whitesPrevFrame - whites))>200){
				cout << "***********************" << endl;

...
namespace
{
	...
	const int maxAccumulatorThreshold = 500;
	...
	void HoughDetection(const Mat& src_gray, const Mat& src_display, int cannyThreshold, int accumulatorThreshold, int inverseResolution)
	{
		...
		HoughCircles(src_gray, circles, CV_HOUGH_GRADIENT, inverseResolution, src_gray.rows / 12, cannyThreshold,
			accumulatorThreshold, 14, 35);
...
{
	CvCapture* capture = cvCreateFileCapture("videos/1.7.mp4");
	long initWhites = 0;
	long whites = 0;

...
long whites = 0;
const int INITIAL_LEARN_FRAME_LIMIT = 20;
...
int getBackgroundThreshold(VideoCapture cap, int limit){
	...
	if (ROI_ENABLED)
		background_gray = background_gray(roi);
	Mat hist_background_gray;
	calcHist(&background_gray, 0, 0, Mat(), hist_background_gray, 1, &histSize, &histRange, uniform, accumulate);
	...
	while (i < limit) {
		...
		if (ROI_ENABLED)
			nextframe_gray = nextframe_gray(roi);
		...
		absdiff(nextframe_gray, nextframe_gray, diffFrame);
		GaussianBlur(diffFrame, diffFrame, Size(20, 20),0,0);
		threshold(diffFrame, diffFrame, 80, 255, cv::THRESH_BINARY);

		//Calculate histograms
		Mat hist_nextFrame_gray;
		calcHist(&nextframe_gray, 0, 0, Mat(), hist_nextFrame_gray, 1, &histSize, &histRange, uniform, accumulate);
		//Model parameters
		double changeCHI = compareHist(hist_nextFrame_gray, hist_background_gray, CHISQUARE);
		double changeBHATTA = compareHist(hist_nextFrame_gray, hist_background_gray BHATTA);
		double changeINTERSEC = compareHist(hist_nextFrame_gray, hist_background_gray, INTERSEC);

		cout << "Change in Red Chi is " << changeCHI << endl;
